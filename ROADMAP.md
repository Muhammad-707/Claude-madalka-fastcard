...
## Фаза 9 — Полировка
- [x] Скелетоны/лоадеры везде, тосты на каждое действие
- [x] Полный mobile-проход (Sidebar drawer, таблицы → карточки)
- [x] A11y, иконки/изображения, единый стиль; `type-check` + `lint` + `build`
- [x] Проверка dark/light на всех экранах
